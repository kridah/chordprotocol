...
class NodeHttpHandler(BaseHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        responsible_node = chord_node.find_successor(chord_node.hash_function(key))

        # If the node is responsible for the key, store it
        if responsible_node.id == chord_node.id:
            ...
            responsible_node = chord_node.find_successor(chord_node.hash_function(key))
            self.send_whole_response(200, responsible_node.address)
...
def run_server(args):
    ...
    global neighbors
    global chord_node
    chord_node = ChordNode(f"localhost:{args.port}")

    # TODO: If there are neighbors provided, we assume the node wants to join an existing network
    # For now, we just initialize the finger table assuming it's the only node in the network
    if not neighbors:
        chord_node.initialize_finger_table(chord_node)
        neighbors.append(chord_node.address)
